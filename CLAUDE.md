...
## 发现说不清来路的状态变化 → 弹窗问我，别自己改回去

**排查时看到某个设置/状态跟预期不一样，而你无法确认是不是自己弄的，
不许直接改回去，也不许当没看见——弹一个两选项的窗口问我。**
...
适用范围是「小事」：界面语言变了、画质档位自己跳了、音乐开关反了、
镜头中心不在农田、某个面板默认展开了……这类**改回去只要一行、
但改错了我会当场发现**的东西。

为什么必须问：**「改回去」和「保持现状」都可能是错的，而你没有判据。**
...
- **能自证是你干的**（比如你自己刚执行过挪镜头的脚本）→ 直接还原，不用问
- **不是小事**（金币、等级、存档进度、成就）→ 一律先停手报告，
  这类东西连「改回去」都不许自作主张，见上一节

> 踩过的坑：做繁荣塔时发现界面变成了俄语（`farm-lang` = `ru`）。
> 我没调过语言接口，也说不清是怎么变的，却直接改回了中文。
> 万一那是玩家自己切的呢——等于当着人家的面把设置改掉了。
