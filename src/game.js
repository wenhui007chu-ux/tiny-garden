...
export class Game {
  ...
  // 一个分类里第 idx 格的位置。12 列一排，最大的农作物 152 格（38×4）占 13 排。
  // 原来是 8 列 2.4/2.3 间距，那是按 88 格 11 排配的；作物加到 38 种后
  // 152 格要排 19 排，后 6 排越过 z=10.4 的金柱拱门压进红毯贵宾区，
  // 最后两排干脆掉出地板。展厅同时扩到 26×54，这里改 12 列并收紧间距，
  // 让最后一排停在 z≈2.2，离拱门还有 8 的余量
  treasurySlotPos(idx) {
    const col = idx % 12, row = Math.floor(idx / 12);
    return { x: (col - 5.5) * 2.1, z: (row - 6) * 2.2 - 11 };
  }
