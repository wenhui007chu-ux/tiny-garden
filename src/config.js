...
// 气象导弹：在观测台发射，把今天的大旱/暴雨强行打成风调雨顺。
// 放在观测台而不是做成商场道具，是因为观测台本来就是管天气的那栋楼，
// 而且「看到今天是暴雨 → 当场一键解决」比「跑去商场买了再回来用」顺得多。
//
// 5 万这个价挺贵：大旱天生长 ×1/3、收成全部生长不良，一天的损失大致就是
// 这个量级，所以它是「今天这局值钱，不想认栽」时才掏的钱，不是每天都放一发。
// 只能改当天，改不了未来——否则观测台报出来的 20 天预报就没意义了。
export const WEATHER_MISSILE = { cost: 50000 };
...
//
// 收入很小，这是有意的：它们是替你推任务进度的，不是新的挂机财源——
// 零点击收入刚砍过两轮，不该从这儿又漏回来。
//
// 定价按各自的每小时产出分档，别再两个都 5 万：
//   垂钓者     每 60 秒一条，ROD 档位内 16 种水产均价 12.5 →   750💰/小时
//   花房管理员 每 180 秒一束，最便宜花束 225           → 4,500💰/小时
// 同价的话花房管理员是垂钓者的整 6 倍，等于垂钓者没人会买。
export const TRAINER_STAFF = [
  ...
  {
    id: 'florist', emoji: '💐', name: '花房管理员', cost: 120000, period: 180,
